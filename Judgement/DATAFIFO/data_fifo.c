...
//
*******************************************************************************************/
int32_t fifo_s_put(fifo_s_t* pfifo, uint8_t element)  //入队单个
{
  //! Check input parameters.
//  ASSERT(NULL != pfifo);
  ...
  pfifo->write_index %= pfifo->buf_size;     //循环fifo
  pfifo->free--;
  pfifo->used++;
...
//
******************************************************************************************/
int32_t fifo_s_puts(fifo_s_t *pfifo, uint8_t *psource, uint32_t number)  //入队多个
{
  int puts_num = 0;
...
//
******************************************************************************************/
uint8_t fifo_s_get(fifo_s_t* pfifo)      //出队
{
  uint8_t   retval = 0;
...
uint16_t fifo_s_gets(fifo_s_t* pfifo, uint8_t* source, uint8_t len)  //出队多个
{
  uint8_t   retval = 0;
